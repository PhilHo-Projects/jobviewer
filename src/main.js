...
function renderBoard() {
    ...
    if (els.newZone) els.newZone.innerHTML = grouped.new.map(renderCard).join('');
    if (els.inProgressZone) els.inProgressZone.innerHTML = grouped.in_progress.map(renderCard).join('');
    if (els.completedZone) els.completedZone.innerHTML = grouped.completed.map(renderCard).join('');
...
function renderCard(job) {
    ...
    let statusBadgeClasses = 'bg-blue-500/10 text-blue-400 border-blue-500/20';
